//! Fast unit tests for security validation
//!
//! These tests directly call validation functions from the source code,
//! avoiding process spawning overhead. They run 100-1000x faster than
//! integration tests while providing better coverage.
...
fn test_path_traversal_detection() {
    ...
    for path in dangerous_paths {
        let result = validators::validate_path_safety(Path::new(path));
...
fn test_json_depth_validation() {
    ...
    assert!(validators::validate_json_structure(&json_bytes, 100, 10_000_000).is_err());

    // Should pass with higher limit
    assert!(validators::validate_json_structure(&json_bytes, 200, 10_000_000).is_ok());
}
...
fn test_json_size_validation() {
    ...
    assert!(validators::validate_json_structure(&json_bytes, 100, 1_000_000).is_err());

    // Should pass with 10MB limit
    assert!(validators::validate_json_structure(&json_bytes, 100, 10_000_000).is_ok());
}
...
proptest! {
    ...
    fn prop_resource_limits_enforced(
        ...
        let result = validators::validate_resource_usage(
            memory,
            time,
            memory_limit,
            time_limit
    ...
    fn prop_json_validation_consistent(
        ...
        let result = validators::validate_json_structure(
            &json_bytes,
            100, // max depth
...
fn benchmark_validation_performance() {
    ...
    for _ in 0..10_000 {
        let _ = validators::validate_path_safety(Path::new("/tmp/test.txt"));
    }
    ...
    let start = Instant::now();
    for _ in 0..10_000 {
        let _ = validators::validate_resource_usage(1000, 100, 10_000_000, 5000);
    }
    ...
    let start = Instant::now();
    for _ in 0..10_000 {
        let _ = validators::validate_json_structure(json_bytes, 10, 1_000_000);
    }
